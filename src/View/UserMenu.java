...
public class UserMenu {
    ...
    public void readNotes(boolean allNot) throws SQLException {
        appManager.readNotes(false);
    }
    ...
    if (choice.equalsIgnoreCase("1")) {
        while (valid) {
            ...
            if (!f.checkIfQuit(password)) {
                attempts += 1;
                Prompts response = appManager.handleNoteAction(NoteAction.REMOVE_ALL, new Note(-1, "", "", null));
                if (response != Prompts.OK) {
                    valid = false;
                    f.prompt(response);
                }
